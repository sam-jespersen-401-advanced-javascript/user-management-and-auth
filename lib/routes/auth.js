...
router
  ...
  .put('/users/:id/roles/:role', ensureAuth(), ensureRole('admin'), (req, res, next) => {
  ...
  .delete('/users/:id/roles/:role', ensureAuth(), ensureRole('admin'), (req, res, next) => {
    if(req.user.id === req.params.id) return 'Insufficient privileges';
    User.updateById(req.params.id, {
      $pull: {
        roles: req.params.role
      }
